...
Method papers should use one of the fourteen categories under the six human context levels in the README. Datasets and benchmarks should follow the sixteen resource categories used in Chapter 7 of the survey.
...
The generated index is stored in `data/resources.json`. Link corrections that are not available in the survey tables are maintained in `data/link_overrides.json`. The README can be regenerated locally with:

```bash
python3 scripts/build_readme.py --survey-root /path/to/survey/source
```

Please do not edit generated tables in `README.md` without updating the corresponding structured source.
